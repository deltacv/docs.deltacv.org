...
Every node in **VisionGraph** is a specialized tool designed for a specific stage of computer vision. To build an effective pipeline, it's important to understand not just what nodes do, but how they communicate through the data type system.
...
VisionGraph uses a **strongly-typed** system to ensure your pipeline is logically sound. You can only connect pins that share the same data type. Each pin features a distinct **icon** that represents the data it carries:
