...
The `processFrame` method from `OpenCvPipeline` must always be overridden — it's where all your vision processing happens. It gets called every time a new frame arrives from the camera (or from a static image or video file, in the case of VisionBench).
